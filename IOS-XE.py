...
# response = requests.get(interfaces_conf, headers=headers,
#                         auth=auth, verify=False)
# if (response.status_code):
...
#         response = requests.get(interfaces_admin_status,
#                                 headers=headers, auth=auth, verify=False)
#         if (response.status_code):
#             admin_status = response.json(
#             )['Cisco-IOS-XE-interfaces-oper:admin-status']

#         interfaces_oper_status = f'{url_base}/data/Cisco-IOS-XE-interfaces-oper:interfaces/interface={name}/oper-status'
#         response = requests.get(interfaces_oper_status,
#                                 headers=headers, auth=auth, verify=False)
#         if (response.status_code):
#             oper_status = response.json(
#             )['Cisco-IOS-XE-interfaces-oper:oper-status']
...
# print('{:<23} {:<14} {:<23} {:<5}'.format(
#     'Interface', 'IP-Address', 'Status', 'Protocol'))

# for i in interfaces:
#     print('{:<23} {:<14} {:<23} {:<5}'.format(
#         i, interfaces[i]['ip'], interfaces[i]['status'], interfaces[i]['protocol']))

# ### Device Status

# device_status = f'{url_base}/data/Cisco-IOS-XE-device-hardware-oper:device-hardware-data/device-hardware/device-system-data'

# response = requests.get(device_status, headers=headers,
#                         auth=auth, verify=False)
# if (response.status_code):
...
#     boot time
#     b_time = parser.parse(device_status['boot-time']).replace(
#         tzinfo=timezone('UTC')).astimezone(timezone('America/Mexico_City'))
#     boot_time = b_time.strftime('%A %d %b %Y %X')

#     print(f'\n--- BOOT TIME ---\n\n{boot_time}')

#     current time
#     c_time = parser.parse(device_status['current-time']).replace(
#         tzinfo=timezone('UTC')).astimezone(timezone('America/Mexico_City'))
...
# response = requests.patch(hostname,  data=payload,
#                           headers=headers, auth=auth, verify=False)
# if (response.status_code):
#     print(response.status_code)
